...
void CGEDebugScreen::DrawAimMode()
{
	...
	if ( localPlayer )
	{
		...
		if ( mode == AIM_NONE )
			_swprintf( wcLine, L"AIM: None; FOV: %0.1f; Zoom: %0.1f", localPlayer->GetFOV(), localPlayer->GetZoom() );
		else if ( mode == AIM_ZOOM_IN )
			_swprintf( wcLine, L"AIM: Zooming In; FOV: %0.1f; Zoom: %0.1f", localPlayer->GetFOV(), localPlayer->GetZoom() );
		else if ( mode == AIM_ZOOM_OUT )
			_swprintf( wcLine, L"AIM: Zooming Out; FOV: %0.1f; Zoom: %0.1f", localPlayer->GetFOV(), localPlayer->GetZoom() );
		else if ( mode == AIM_ZOOMED )
			_swprintf( wcLine, L"AIM: Zoomed; FOV: %0.1f; Zoom: %0.1f", localPlayer->GetFOV(), localPlayer->GetZoom() );
