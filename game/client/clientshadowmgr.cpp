...
void CClientShadowMgr::InitDepthTextureShadows()
{
	...
	// GE_DLL: set m_nDepthTextureResolution to the depth resolution we want
	m_nDepthTextureResolution = r_flashlightdepthres.GetInt();
...
#else
		// GE_DLL: we want to create a render target of specific size, so use RT_SIZE_NO_CHANGE
...
#else
			// GE_DLL: we want to create a *DEPTH TEXTURE* of specific size, so use RT_SIZE_NO_CHANGE and MATERIAL_RT_DEPTH_ONLY
...
//			depthTex.InitRenderTarget( m_nDepthTextureResolution, m_nDepthTextureResolution, RT_SIZE_OFFSCREEN, dstFormat, MATERIAL_RT_DEPTH_NONE, false, strRTName );
			...
			// GE_DLL: ensure the depth texture size wasn't changed
