...
CGEMPPlayer* BotPutInServer( bool bFrozen, int iTeam )
{
	...
	CGEMPPlayer *pPlayer = ((CGEMPPlayer *)CBaseEntity::Instance( pEdict ));
	pPlayer->ClearFlags();
	...
	CCommand teamcmd;
	teamcmd.Tokenize( cmd );

	pPlayer->ClientCommand( teamcmd );

	if ( GERules()->IsTeamplay() )
	{
		if ( pPlayer->GetTeamNumber() == TEAM_JANUS )
		{
			Q_snprintf( cmd, sizeof( cmd ), "joinclass oddjob" );
			pPlayer->SetCollisionGroup( COLLISION_GROUP_JANUS );
		}
		else
		{
			Q_snprintf( cmd, sizeof( cmd ), "joinclass mishkin" );
			pPlayer->SetCollisionGroup( COLLISION_GROUP_MI6 );
		}
	}
	else
	{
		Q_snprintf( cmd, sizeof( cmd ), "joinclass bond 2" );
		pPlayer->SetCollisionGroup( COLLISION_GROUP_PLAYER );
	}

	CCommand classcmd;
	classcmd.Tokenize( cmd );

	pPlayer->ClientCommand( classcmd );
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CGEPlayer *fakeclient, const QAngle& viewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, float frametime )
{
	if ( !fakeclient )
		return;
...
void Bot_Think( CGEPlayer *pBot )
{
	...
	if ( pBot->IsAlive() && (pBot->GetSolid() == SOLID_BBOX) )
	{
		...
		// Stop when shot
		if ( !pBot->IsEFlagSet(EFL_BOT_FROZEN) || bot_flipout.GetInt() )
		{
			if ( pBot->m_iHealth == MAX_HEALTH )
			{
		...
		if ( (!pBot->IsEFlagSet(EFL_BOT_FROZEN) && pBot->m_iHealth == MAX_HEALTH) || bot_flipout.GetInt() )
		{
			Vector vecEnd;
			Vector forward;
			...
			while ( --maxtries >= 0 )
			{
				...
				botdata->nextturntime = gpGlobals->curtime + 2.5;
	...
	else
	{
		...
		if ( !pBot->IsAlive() && !pBot->IsObserver() )
		{
